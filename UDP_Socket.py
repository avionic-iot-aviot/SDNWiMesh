...
def SendUdpPacketUnicastLoop(port,src,dst):
   ...
   while True:
      ...
      pckReort = ReportPacket ( config.get(socket.gethostname(),'NetId'), config['GENERAL']['IpSink'], src, "100", dst, config.get(socket.gethostname(),'Id') + " " + ', '.join(node_variables.list_neighbor) )
