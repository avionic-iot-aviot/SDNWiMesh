...
class Packets:
    def __init__(self, NetId, Length, Destination, Source, Type, TTL, NextHop,
                 Payload):
        self.NetId = NetId
        self.Length = Length
    ...
    def getPacketFromBytes(cls, frame):
        ...
        if isinstance(frame8, str):
            return cls(frame1.decode("utf-8"), frame2.decode("utf-8"),
                       frame3.decode("utf-8"), frame4.decode("utf-8"),
                       frame5.decode("utf-8"), frame6.decode("utf-8"),
                       frame7.decode("utf-8"), frame8.decode("utf-8"))
        else:
            return cls(frame1.decode("utf-8"), frame2.decode("utf-8"),
                       frame3.decode("utf-8"), frame4.decode("utf-8"),
                       frame5.decode("utf-8"), frame6.decode("utf-8"),
                       frame7.decode("utf-8"), frame8)

    def getBytesFromPackets(self):
        self.fixTheLen()

        if ((len(self.NetId) == int(config['PACKET']['LenNetId']))
                and (len(self.Length) == int(config['PACKET']['LenLength']))
                and
            (len(self.Destination) == int(config['PACKET']['LenDestination']))
                and (len(self.Source) == int(config['PACKET']['LenSource']))
                and (len(self.Type) == int(config['PACKET']['LenType']))
                and (len(self.TTL) == int(config['PACKET']['LenTTL']))
                and (len(self.NextHop) == int(config['PACKET']['LenNextHop']))
                and
            (len(self.Payload) <= int(config['PACKET']['LenPayload']))):
    ...
    def LengthCalcolator(self):
        temp = len(self.NetId) + len(self.Destination) + len(
            self.Source) + len(self.Type) + len(self.TTL) + len(
                self.NextHop) + len(self.Payload)
        #print(temp)
...
class ReportPacket(Packets):
    def __init__(self, NetId, Destination, Source, TTL, NextHop, Payload):
        super().__init__(NetId, "", Destination, Source, "1", TTL, NextHop,
                         Payload)  #1 report
        super().LengthCalcolator()
...
class DataPacket(Packets):
    def __init__(self, NetId, Destination, Source, TTL, NextHop, Payload):
        ...
        super().__init__(NetId, "", Destination, Source, "2", TTL, NextHop,
                         Payload)  #2 data
        super().LengthCalcolator()
